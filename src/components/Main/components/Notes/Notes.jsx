...
import { deleteBlock } from '../../../../store/notesSlice';
            ...
            <table className='note'>
                {notes.map((note, id) => {
                    return <tr className="doc" key={id}>
                        ...
                        {editBlock && <td className="close-note-td"><div className="close-note" onClick={() => dispatch(deleteBlock(id))}> <p>X</p> </div></td>}
                    </tr>
                })}
            </table>
            {showModal && <AddNote close={handleModalClose} />}
        </div >
    )
}
